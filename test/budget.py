...
class Node :
	def __init__(self, app, nodeIndex) :
		...
		self._trbod = self._app.getMonetaryunitd()
		self._trboCli = self._app.getMonetaryunitCli()
		self._daemonProcess = None

	def createDataDir(self, nodeCount, masterNodePrivateKey = None) :
		makeDirs(self._nodePath)
		writeFile(
			os.path.join(self._nodePath, 'trbo.conf'),
			self._generateMonetaryunitConf(nodeCount, masterNodePrivateKey)
		)
		
	def startNode(self) :
		self._daemonProcess = subprocess.Popen([ self._trbod, '-datadir=' + self._nodePath, '-daemon' ])
	...
	def executeCli(self, *args) :
		...
		output = executeCommand(self._trboCli, '-datadir=' + self._nodePath, *normalizedArgs)
...
class Application :
	...
	def _setup(self) :
		...
		self._trbod = os.getenv('MONETARYUNITD', None)
		if not self._trbod :
			die('Undefined MONETARYUNITD')
		self._trboCli = os.getenv('MONETARYUNITCLI', None)
		if not self._trboCli :
			die('Undefined MONETARYUNIT')
		print('trbod: %s' % (self._trbod))
