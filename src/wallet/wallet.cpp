...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_5000000);
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == 5000000 * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (IsLockedCoin((*it).first, i) && nCoinType != ONLY_5000000)
                    continue;
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, CoinSelectStrategy coinSelectStrategy, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 TRBO
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 TRBO over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        //if(out.tx->vout[out.i].nValue == 5000000*COIN) continue;
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            bool fAccepted = false;

            // Function returns as follows:
            //
            // bit 0 - 10000 TRBO+1 ( bit on if present )
            // bit 1 - 1000 TRBO+1
            // bit 2 - 100 TRBO+1
            // bit 3 - 10 TRBO+1
            // bit 4 - 1 TRBO+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nObfuscationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT5000000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (fMasterNode && out.tx->vout[out.i].nValue == 5000000 * COIN) continue; //masternode input
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, GetBudgetSystemCollateralAmount(chainActive.Height()))); // Old 50 TRBO collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, param->coinSelectStrategy, coinControl, coin_type, useIX)) {
                    ...
                    } else if (coin_type == ONLY_NOT5000000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 5000000 TRBO.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT5000000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 5000000 TRBO.");
                    } else {
