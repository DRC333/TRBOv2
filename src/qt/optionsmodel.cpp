...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTRBOAmount"))
        settings.setValue("nAnonymizeTRBOAmount", 1000);

    nAnonymizeTRBOAmount = settings.value("nAnonymizeTRBOAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTRBOAmount"))
        SoftSetArg("-anonymizetrboamount", settings.value("nAnonymizeTRBOAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTRBOAmount:
            return QVariant(nAnonymizeTRBOAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTRBOAmount:
            nAnonymizeTRBOAmount = value.toInt();
            settings.setValue("nAnonymizeTRBOAmount", nAnonymizeTRBOAmount);
            emit anonymizeTRBOAmountChanged(nAnonymizeTRBOAmount);
            break;
        case CoinControlFeatures:
